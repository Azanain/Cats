...
public class EquipmentPlace : MonoBehaviour
{
    ...
    public float NumberUpgradesThisType;// { get; private set; }
    ...
    private void OnEnable()
    {
        if(_eventManager != null)
         _eventManager.UpgradeRestaurantIsBoughtEvent += CheckTypeEquipmentForUpgrade;
    }
    private void Start()
    {
        LoadData();
    ...
    {
        // загружать Sublevel при выборе ресторана
        //по количеству улучшений этого ресторана сделать CheckTypeEquipmentForUpgrade по каждому типу
    }
    private void CheckTypeEquipmentForUpgrade(EquipmentTypes type)
    {
        if (equipmentInfo.EquipmentTypes == type)
        {
            NumberUpgradesThisType = _bank.GetNumberAllUpgradeTypeInrestaurant(EquipmentManager.InfoRestaurant.name, type);

            Debug.Log($"Get upgrade on {name}, info = {equipmentInfo}");
        }
        else
        {
            NumberUpgradesThisType = 0;
            Debug.Log($"Not upgrade on {name}, info = {equipmentInfo}");
        }
    }
